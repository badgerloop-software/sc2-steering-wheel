...
void initDisplay(bool SD_enable) {
  ...
  // each peripheral digitalWrite(22, HIGH); // Touch controller chip select (if
  // used)
  ...
  tft.setRotation(1); // Somehow important to have an odd rotation number
                      // Display does not work properly if you do not set this
                      // and initialise the SD card
...
static void drawStatusPill(int x, int y, int w, int h, const char *label,
                           bool value, uint16_t color) {
...
void renderMinimalDisplay(float speed) {
  ...
  int accelPercent =
      (int)lroundf(((accelVoltage - 0.618f) / (2.814f - 0.618f)) * 100.0f);
  if (accelPercent < 0) {
    accelPercent = 0;
  ...
  drawStatusPill(128, 6, 82, 28, modeText, currentDriveMode,
                 currentDriveMode ? TFT_RED : TFT_GREEN);
  ...
  tft.setTextSize(3);
  tft.setCursor(10, 90);
  snprintf(odoBuffer, sizeof(odoBuffer), "%lu.%lu",
           (unsigned long)(currentOdo / 10), (unsigned long)(currentOdo % 10));
  tft.print(odoBuffer);
